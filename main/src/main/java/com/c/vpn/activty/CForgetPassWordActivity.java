...
public class CForgetPassWordActivity extends CBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cforget_pass_word);
        initActionBar();
        initViews();
    }
        ...
        tvCode.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(TextUtils.isEmpty(username)){
                    ToastUtils.showToastLong(CForgetPassWordActivity.this,"邮箱不能为空");
                    return;
                }
                if(!CommmonUtil.isEmail(username)){
                    ToastUtils.showToastLong(CForgetPassWordActivity.this,"请填写正确的邮箱格式");
                    return;
                }
                CForgetPassWordActivity.requestVerifyCode task = new CForgetPassWordActivity.requestVerifyCode();
        ...
        btnReg.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(TextUtils.isEmpty(code)){
                    ToastUtils.showToastLong(CForgetPassWordActivity.this,"验证码不能为空");
                    return;
                }
                String password = etPassword.getText().toString();
                if(TextUtils.isEmpty(password)){
                    ToastUtils.showToastLong(CForgetPassWordActivity.this,"密码不能为空");
                    return;
                }
                if(!CommmonUtil.checkPasswordRule(password)){
                    ToastUtils.showToastLong(CForgetPassWordActivity.this,"密码必须包含大小写字母及数字且不低于6位");
                    return;
                }
                showDialog();
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        JSONObject result = HttpUtil.sendGet(Url.CHANGEPWD+"?id="+username+"&verifycode="+code+"&newpassword="+password+"&code=1");
                        if(result != null){
    ...
    public void onReg(RegEvent event) {
        ...
        if(obj.getIntValue("code") != 0){
            ...
            ToastUtils.showToastLong(CForgetPassWordActivity.this,"重置密码成功");
            //这里自动登录逻辑
            new Thread(new Runnable() {
                @Override
                public void run() {
                    ...
                    if(result.getIntValue("code") != 0){
                        ToastUtils.showOnUIThreadx(CForgetPassWordActivity.this,result.getString("desc"));
                    }else{
    ...
    public class requestVerifyCode implements HttpAsyncRequest.HttpAsyncCallback {
        ...
        public void request (String username) {
            ...
            task.execute(Url.CODE+"?id="+username+"&method=1");
        }
        @Override
